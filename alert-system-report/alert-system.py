...
# Here, we set up an anomaly detection system using Airflow.
# Every 15 minutes, the system performs an ETL task and reports 
# via corporate messenger chat (Telegram in this case) using a bot
# in case anomaly was found in users amount, likes, views number, or CTR.
# Anomaly is any value lying outside an interval of
# (Q1 - IQR * coeff; Q3 + IQR * coeff)
# where coeff is a sensitivity coefficient set for every metric
# (usually from 0.5 to 1.5)

# An example of the report is shown in the files 'alert-system-report.jpg'
# and 'alert-system-report-text.png'
...
bot = telegram.Bot(token=bot_token) # access to the bot
...
class Extractor:
    """
    Extracts a dataframe, removes the last timestamp value for more correct comparisons
    """
    @staticmethod
