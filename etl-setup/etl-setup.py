...
# Here, we set up an ETL pipeline in Airflow by creating a DAG 
# that extracts the data from the feed and messenger databases,
# transforms it to have for every date and users feature (os type, gender, age)
# aggregated users activity - likes, views, messages, and users contacted.
# Finally, the DAG uploads the resulting dataframe into a new database schema.

# An example of the table is shown in the folder in the file called 'etl-setup-table.png'
...
# Load environment variables (the passwords) from .env file
load_dotenv()
...
# list with the columns - needed after we do the join at the transform step
sorted_cols = [
    'event_date',
    'user_id',
    'os',
